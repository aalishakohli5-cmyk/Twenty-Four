...
export default function RewardStore() {
  ...
  async function load() {
    try {
      ...
      const auroraReward = (rewards as Reward[]).find((reward) => reward.name === "Aurora") ?? rewards[0] ?? null;
      setAurora(auroraReward);
      setBalance(wallet.balance);
      const savedTheme = getDashboardTheme();
      const serverTheme: DashboardTheme = auroraReward?.active ? "aurora" : "midnight";
      if (savedTheme !== serverTheme) applyDashboardTheme(serverTheme);
  ...
  async function chooseDefault() {
    ...
    setMessage("");
    try {
      await api.post("/api/rewards/deactivate", { category: "theme" });
      applyDashboardTheme("midnight");
      setTheme("midnight");
      setMessage("Midnight theme applied.");
      await load();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "The theme could not be changed");
    } finally {
      setWorking(false);
    }
  }
  ...
  return <div className="dashboard-shell marketplace-shell"><Navbar/><main className="dashboard-main marketplace-main">
    <header className="marketplace-header">
      ...
      <div className="market-balance"><Coins size={20}/><span><small>AVAILABLE</small><strong>{loading ? "Connecting…" : `${balance} coins`}</strong></span></div>
    </header>
    ...
    <section className="theme-market-grid" aria-busy={loading}>
      <article className={`theme-product midnight-product ${theme === "midnight" ? "selected" : ""}`}>
        ...
        <footer><strong>Free</strong><button onClick={chooseDefault} disabled={loading || working || theme === "midnight"}>{theme === "midnight" ? <><Check/> In use</> : "Use theme"}</button></footer>
      </article>
      ...
      <article className={`theme-product aurora-product ${theme === "aurora" ? "selected" : ""}`}>
        ...
        <footer><strong>{loading ? "Waking marketplace…" : `${price} coins`}</strong><button onClick={unlockOrUseAurora} disabled={loading || working || theme === "aurora" || (!aurora?.owned && !canAfford)}>{loading ? "Please wait" : theme === "aurora" ? <><Check/> In use</> : aurora?.owned ? "Use theme" : canAfford ? "Unlock Aurora" : <><LockKeyhole/> Need {price - balance}c</>}</button></footer>
      </article>
    </section>
